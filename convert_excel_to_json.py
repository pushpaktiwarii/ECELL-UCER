...
# Read the Excel file
df = pd.read_excel('Participants Certificates.xlsx')
...
for record in data:
    # Convert phone number to string and remove scientific notation and spaces
    if 'Phone' in record and pd.notna(record['Phone']):
        phone_str = str(record['Phone']).replace(' ', '').replace('-', '')
        try:
            # Try to convert to int to remove scientific notation, then back to string
            record['Phone'] = str(int(float(phone_str)))
        except (ValueError, TypeError):
            # If conversion fails, just use the cleaned string
